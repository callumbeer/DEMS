	...
	var ISSelectReplacement = {
		...
		add_option: function(select, option, id)
		{
			...
			if($(option).hasClass('freeform')) {
				...
				html += '<input type="checkbox" class="categories" name="ISSelectReplacement_'+select.name+'['+extraKey+']'+extra+'" value="'+option.value+'" '+checked+disabled+'" onclick="ISSelectReplacement.on_click(this, \''+id+'\');" />' + label;
			}
			html += '</li>';
			return html;
		},
